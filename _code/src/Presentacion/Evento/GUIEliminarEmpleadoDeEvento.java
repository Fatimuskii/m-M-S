...
*/
public class GUIEliminarEmpleadoDeEvento extends GUIEvento{

	/**
	 * 
	 */
	private static final long serialVersionUID = 6600304960578369024L;
	private JPanel contentPane;
	private JTextField textEmpleado;
	private JTextField textEvento;

	public GUIEliminarEmpleadoDeEvento() {
		super();
	...
	*/
	private void initGUI() {
		setTitle("Eliminar Empleado de Evento");
		...
		btnAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					Controlador.getInstance().accion(new Contexto(Events.ELIMINAR_EMPLEADO_EVENTO, tParticipa));
	...
	*/
	...
	public void actualizar(Contexto contexto) {
		...
		case Events.ELIMINAR_EMPLEADO_EVENTO_OK:
			JOptionPane.showMessageDialog(null, "Empleado " +  " eliminado del Evento " );
			this.dispose();
			break;
		case Events.ELIMINAR_EMPLEADO_EVENTO_KO:
			if((int)contexto.getDato() == -1){
				...
				JOptionPane.showMessageDialog(null, "El empleado ya está desasignado al evento");
			}
			else if((int)contexto.getDato() == -6){
				JOptionPane.showMessageDialog(null, "El empleado no está asignado a ese evento");
			}
			else{
				JOptionPane.showMessageDialog(null, "Error al eliminar el empleado del evento");
			}
			break;
		}
