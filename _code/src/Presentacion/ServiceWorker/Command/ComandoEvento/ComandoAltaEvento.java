...
*/
public class ComandoAltaEvento implements Command {
	/** 
	* (non-Javadoc)
	...
	*/
	...
	public Contexto ejecutar(Object dato) {
		...
		int id = FactoriaSA.getInstance().crearSAEvento().altaEvento(tEvento);
		
		if(id > 0){
			return new Contexto(Events.ALTA_EVENTO_OK, id);
		}
		else{
			return new Contexto(Events.ALTA_EVENTO_KO, id);
		}
	}
}