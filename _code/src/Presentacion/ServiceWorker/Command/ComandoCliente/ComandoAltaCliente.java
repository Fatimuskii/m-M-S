...
public class ComandoAltaCliente implements Command {
	...
	public Contexto ejecutar(Object dato) {
		int id =FactoriaSA.getInstance().crearSACliente().altaCliente((TCliente) dato);
		
		if(id > 0)
			return new Contexto(Events.ALTA_CLIENTE_OK, id);
		else
			return new Contexto(Events.ALTA_CLIENTE_KO, id);
	}
}