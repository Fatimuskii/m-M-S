...
public class ComandoModificarEmpleado implements Command{
	...
	*/
	...
	public Contexto ejecutar(Object dato) {
		...
		int id = FactoriaSA.getInstance().crearSAEmpleado().modificarEmpleado(tEmpleado);
		
		if(id > 0){
			return new Contexto(Events.MODIFICAR_EMPLEADO_OK, id);
		}
		else{
			return new Contexto(Events.MODIFICAR_EMPLEADO_KO, id);
		}
	}
}
